...
from flask import (Flask, render_template, request, redirect, url_for,
                   Response, jsonify, current_app, session, send_file)
...
emotion_descriptions = {
    "happy": ("Feeling joyous and content. "
              "Happiness is a positive emotion that arises "
              "from experiencing pleasure, satisfaction, or success. "
              "It can be expressed through smiling, laughter, "
              "and a generally upbeat demeanor. Happiness contributes "
              "to overall well-being and can be contagious, "
              "spreading positivity to those around us."),
    "sad": ("Feeling sorrowful or unhappy. Sadness is a natural "
            "human emotion often triggered by loss, disappointment, "
            "or failure. It's characterized by feelings of grief, "
            "loneliness, or despair. While sadness can be "
            "uncomfortable, it's important to acknowledge and express "
            "these emotions, as doing so can lead to healing "
            "and emotional growth."),
    "angry": ("Feeling intense displeasure or hostility. Anger "
              "is a powerful emotion that arises in response to perceived "
              "threats, injustices, or frustrations. It's often accompanied "
              "by physical sensations like increased heart rate and muscle "
              "tension. While anger itself is not inherently negative, "
              "it's important to manage it constructively to avoid harmful "
              "behaviors and maintain healthy relationships."),
    "fear": ("Feeling scared or anxious. Fear is an adaptive "
             "response to perceived danger or threat. It triggers a fight, "
             "flight, or freeze reaction in the body, preparing us to "
             "respond to potential harm. While fear can be protective, "
             "excessive or irrational fear can interfere with daily life. "
             "Overcoming fear often involves facing it gradually and "
             "developing coping strategies to manage anxiety."),
    "surprise": ("Feeling astonished or amazed. Surprise is a brief, "
                 "intense emotion that occurs in response to unexpected "
                 "events or stimuli. It's characterized by a sudden shift "
                 "in attention and physiological arousal. Surprise can be "
                 "positive or negative depending on the context, but it "
                 "adds excitement and novelty to our experiences."),
    "neutral": ("Showing no particular emotion. Neutrality is a "
                "state of emotional calmness and balance where no strong "
                "feelings or reactions are present. It can be a temporary "
                "state or a habitual response to certain situations. "
                "While neutrality may seem unremarkable, it can serve "
                "as a valuable buffer against stress and overwhelm."),
    "disgust": ("Feeling revulsion or strong disapproval. Disgust "
                "is an aversive emotion triggered by offensive or repulsive "
                "stimuli, such as foul odors, unsanitary conditions, "
                "or morally objectionable behavior. It serves as a protective "
                "mechanism to avoid potentially"
                "harmful substances or situations. "
                "While disgust may be unpleasant, it plays an important "
                "role in maintaining hygiene and social norms.")
}
...
def deepface_frame(frame):
    ...
    This function analyzes the emotion in the provided
    image using the DeepFace library.
    It detects facial attributes such as emotion,
    using the OpenCV backend, and returns
    the analysis result.
    ...
    Returns:
        ...
        including the detected emotions.
    """
...
def get_emoji(emotion):
    """
    Get the path to the emoji image corresponding
    to the given emotion.

    Args:
        emotion (str): The emotion for which to
        retrieve the emoji path.
...
def draw_cursor(img, x, y, region):
    """
    Draw a cursor on the image and check if it's near
    the target position.
    ...
    Args:
        ...
        region (numpy.ndarray): The region of interest
        for overlaying the exit icon.

    Returns:
        bool: True if the cursor is not near the target position,
        False otherwise.
    """
    ...
    if ((x <= xs <= x + region.shape[1]) and
            (y <= ys <= y + region.shape[0])):
        cv2.putText(img, "Done", (1580, 260),
...
def generate_frames(name="asset"):
    """
    Generates frames from webcam feed with emotion analysis
    and overlay of emoji.

    This function continuously captures frames from the webcam feed,
    analyzes each frame for emotion using the DeepFace library,
    overlays an emoji image corresponding to the detected
    emotion onto the frame, and adds text indicating
    the detected emotion to the frame.
    The frames are encoded as JPEG images
    ...
    while True:
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                ...
                if face_analysis is not None:
                    ...
                    if emoji is not None:
                        ...
                        alpha_l = 1.0 - alpha_s
                        for c in range(0, 3):
                            frame[y1:y2, x1:x2, c] = (alpha_s
                                                      * emoji[:, :, c]
                                                      + alpha_l
                                                      * frame[y1:y2, x1:x2, c])

                        x_offset_exit = frame.shape[1] - exit.shape[1] - 20
                        # 20 pixels from the right edge
                        y_offset_exit = 20

                        # Define the region of interest
                        # for overlaying the exit icon
                        exit_roi = (frame[y_offset_exit:y_offset_exit
                                    + exit.shape[0],
                                    x_offset_exit:x_offset_exit
                                    + exit.shape[1]])

                        leave = draw_cursor(frame, x_offset_exit,
                                            y_offset_exit,
                                            exit_roi)

                        cv2.putText(frame, "Exit", (x_offset_exit + 200,
                                                    y_offset_exit + 10),
                        ...
                        if leave is False:
                            with app.app_context():
                                ...
                                df.to_csv(f'{name}_emotion_data.csv',
                                          index=False)
            ...
            cv2.putText(frame, f"Emotion: {emotion}", (40, 200),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 245, 238), 2)
            cv2.putText(frame, f"Emotion: {emotion}", (42, 202),
...
def video():
    ...
    and streams them as a multipart
...
def analyze():
    ...
    This function receives an image file
    uploaded via a POST request,
    saves it as 'temp_image.jpg', analyzes the emotion
    in the image using DeepFace,
    and renders the 'result.html' template with the image
    and the dominant emotion.

    Returns:
        str: Rendered HTML template
        with image and dominant emotion.
    """
    if (request.files['file']):
        ...
        pict = "static/picture/temp_image.jpg"
        face_analysis = DeepFace.analyze(img_path=pict)
    ...
    random = "https://example.com/default_emoji.gif"
    emoji_url = emoji_urls.get(emotion,
                               random)
...
def emotion():
    """
    This function processes emotion data obtained
    from a CSV file and generates visualizations.
    It reads the CSV file based on the username
    provided in the form,
    groups the data by emotion,
    and creates bar charts and time series
    graphs to visualize the
    occurrence of each emotion over time.
    ...
    time_series_fig = go.Figure(data=time_series_traces,
    ...
    emotion_descriptions = {
        "happy": ("Patient is feeling joy and satisfaction.\n"
                  "Keep on going. \n"
        ...
        "fear": ("Patient feeling stressed out. Need to take a break! \n"
                 "Reassure them that it's okay to feel afraid and \n"
                 "that they are not alone."),
        "disgust": ("Feeling a strong aversion or repugnance. \n"
                    "Listen to their concerns without judgment."),
        "neutral": ("Feeling normal. Keep on going. You got it! Be \n"
                    "attentive and observant, as they may be concealing \n"
                    "their true feelings."),
        "angry": ("Very upset. Need some cooldown. \n "
                  "Give them some tea or ice cream!")
    }
    ...
    return render_template('emotion.html', bar_plot_div=bar_plot_div,
                           time_series_plot_div=time_series_plot_div,
